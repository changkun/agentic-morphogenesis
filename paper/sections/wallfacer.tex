...
Methodologically, this is an adaptive observational study rather than a randomized experiment. Architectural transitions were chosen online in response to observed failure modes, so the section provides evidence about one realized trajectory, not clean causal estimates of what each added role would have done under controlled counterfactual conditions.

\TODO{Add a reproducibility appendix with exact prompts, model/version identifiers, token and time budgets, restart policy, logging schema, and the operational rule used to classify a bottleneck as ``clear and persistent.'' Without these details, the Wallfacer study remains difficult to reproduce.}
